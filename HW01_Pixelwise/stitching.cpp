...
void blend_stitching(const Mat l1, const Mat l2, Mat &l_f, int diff_x, int diff_y, float alpha);
...
int main() {
	...
	cout << "bound_u: " << bound_u << "bound_b " << bound_b << endl;
	cout << "bound_l: " << bound_l << "bound_r " << bound_r << endl;
	...
	for (int i = -diff_x_; i < I_f.rows - diff_x_; i++) {
		for (int j = -diff_y_; j < I_f.cols - diff_y_; j++) {
			...
			if (x1 >= 0 && x2 < I2_row && y1 >= 0 && y2 < I2_col){
				for (int k = 0; k < 3; k++) {
					float f1 = mu * I2.at<Vec3f>(x1, y2)[k] + (1 - mu) * I2.at<Vec3f>(x1, y1)[k];
					float f2 = mu * I2.at<Vec3f>(x2, y2)[k] + (1 - mu) * I2.at<Vec3f>(x2, y1)[k];
