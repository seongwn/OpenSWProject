...
int nearestNeighbor2nd(Mat& vec, vector<KeyPoint>& keypoints, Mat& descriptors, int nn);
...
void findPairs(vector<KeyPoint>& keypoints1, Mat& descriptors1, vector<KeyPoint>& keypoints2, Mat& descriptors2,
	...
	for (int i = 0; i < descriptors1.rows; i++) {
		...
		if (ratio_threshold) {
			//
			int nn2 = nearestNeighbor2nd(desc1, keypoints2, descriptors2, nn);
		...
		if (crossCheck) {
			//
			if (nearestNeighbor(descriptors2.row(nn), keypoints1, descriptors1) != i) {
				continue;
			}
			//
		}
